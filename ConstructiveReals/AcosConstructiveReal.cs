...
public class AcosConstructiveReal : ConstructiveReal
{
    private ConstructiveReal _op;
    private ConstructiveReal? _reduced;
    private object _lock = new();

    public AcosConstructiveReal(ConstructiveReal x)
    {
        _op = x;
    }
    ...
    private void Reduce(ConstructiveRealEvaluationSettings es)
    {
        lock (_lock)
        {
            if (_reduced == null)
            {
                _reduced = new AsinConstructiveReal(_op).Negate().Add(es.Factory.Pi().Shift(-1));
            }
        }
    }
